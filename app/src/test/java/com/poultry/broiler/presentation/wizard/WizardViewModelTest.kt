...
class WizardViewModelTest {
    ...
    private fun viewModel(): WizardViewModel = WizardViewModel(
    ...
    private val existingDimensions = HouseDimensions(
        id = "id-existing",
        projectId = "p-1",
        length = Meters(100.0),
        width = Meters(12.0),
    ...
    @Test
    fun load_emitsActiveWithRestoredDimensions() = runTest {
        every { getUseCase("p-1") } returns flowOf(existingDimensions)
        val vm = viewModel()
        vm.load("p-1")
        vm.uiState.test {
            // Allow the load coroutine to complete via launch
            val firstActive = (awaitItem() as? WizardUiState.Active) ?: WizardUiState.Active(
                currentStep = 1,
                totalSteps = 6,
    ...
    @Test
    fun onIntent_updateLength_recomputesFloorArea() = runTest {
    ...
    @Test
    fun onIntent_selectRoofTypePitched_clearsRidgeHeightWhenSwitchingAway() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
        vm.load("p-1")
    ...
    @Test
    fun onIntent_selectInsulationNone_clearsThickness() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
        vm.load("p-1")
    ...
    @Test
    fun onIntent_goNextValid_advancesStep() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
        vm.load("p-1")
        fillValidForm(vm)
    ...
    @Test
    fun onIntent_goNextWithInvalidForm_blocksAdvanceAndProducesFieldErrors() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
        vm.load("p-1")
    ...
    @Test
    fun onIntent_updateLengthNegative_surfacesFieldError() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
        vm.load("p-1")
    ...
    @Test
    fun computeState_loadingFirst_thenActive() = runTest {
        every { getUseCase("p-1") } returns flowOf(null)
        val vm = viewModel()
